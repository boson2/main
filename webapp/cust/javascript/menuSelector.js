...
var aMenuSelector = null;
MenuSelector = function(isTree)
{
		...
		if( this.m_isTree == true ) {
			...
			if( checkIds != null && checkIds.length > 0 ) {
				...
				if( i > 0 ) {
					...
					this.m_ajax.send("POST", portalPage.getContextPath() + "/userMenu/addMyMenusOnlyForAjax.face", param, false, {
			...
			if( ix > 0 ) {
				...
				this.m_ajax.send("POST", portalPage.getContextPath() + "/userMenu/addMyMenusForAjax.face", param, false, {
					success: function(data){
						location.reload();
						//enviewMessageBox.doShow( portalPage.getMessageResource('pt.ev.message.success.add') );
					}});
